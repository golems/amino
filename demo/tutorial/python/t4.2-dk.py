...
q_all = sg.config_vector( {"s1": -.75*pi,
                           "e": .75*pi,
                           "f0": .125*pi,
                           "f1": .125*pi,
                           "f2": .125*pi } )
...
while win.is_runnining():
    ...
    tfvel.set_translation((dx, 0, 0))
    dk.set_ref_tf_vel(tfvel)

    # Set the reference pose.  If we ommit position feedback, the
    # robot will drift over time due to floating point error.
    tf_ref = QuatTrans(( tf0.rotation(),
                         (tf0.translation() + (x,0,0)) ))
    dk.set_ref_tf(tf_ref)
